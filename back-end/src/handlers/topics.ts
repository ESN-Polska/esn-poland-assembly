...
class Topics extends ResourceController {
  ...
  protected async getResources(): Promise<Topic[]> {
    ...
    if (!this.galaxyUser.hasPermission('qa')) topics = topics.filter(x => !x.isDraft());
  ...
  protected async getResource(): Promise<Topic> {
    if (this.topic.isDraft() && !this.galaxyUser.hasPermission('qa')) throw new HandledError('Unauthorized');
