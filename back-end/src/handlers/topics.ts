...
class Topics extends ResourceController {
  ...
  protected async getResources(): Promise<Topic[]> {
    ...
    if (!this.galaxyUser.hasPermission('qa.topics')) topics = topics.filter(x => !x.isDraft());
  ...
  protected async postResources(): Promise<Topic> {
    if (!this.galaxyUser.hasPermission('qa.topics')) throw new HandledError('Unauthorized');
  ...
  private async insertFromApplication(
    ...
    if (!this.galaxyUser.hasPermission('qa.topics')) throw new Error('Unauthorized');
  ...
  protected async getResource(): Promise<Topic> {
    if (this.topic.isDraft() && !this.galaxyUser.hasPermission('qa.topics')) throw new HandledError('Unauthorized');
